...
def check_cluster_connectivity(namespace="default"):
    """Verify cluster connectivity before monitoring"""
    try:
        ...
        print(f"✅ Monitoring in context: {current_context}")

        # Check if the specified namespace exists
        subprocess.run(['kubectl', 'get', 'namespace', namespace], capture_output=True, check=True)
...
def main():
    ...
    parser.add_argument('--context', help='Switch to a specific context before monitoring')
    parser.add_argument('--validate-connectivity', action='store_true', help='Validate cluster connectivity before monitoring')
    ...
    if args.validate_connectivity or not args.analyze_controller:
        if not check_cluster_connectivity(args.namespace):
            print("❌ Cannot proceed without cluster connectivity")
            return
