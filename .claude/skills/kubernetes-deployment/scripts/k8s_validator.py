...
def check_kubernetes_connection():
    ...
    try:
        ...
        print(f"✅ Connected to cluster context: {current_context}")
        return True
    except subprocess.CalledProcessError as e:
        print(f"⚠️  Could not connect to Kubernetes cluster: {e}")
        print("Validation will proceed without cluster connectivity check.")
        return False
...
def main():
    ...
    parser.add_argument('--cluster-check', action='store_true', help='Check cluster connectivity before validation')

    args = parser.parse_args()

    # Perform cluster check if requested
